...
class SsePCDDataset(PointCloudDataset):
    """Class to handle SsePCD dataset."""

    def __init__(self, config, set='training', use_potentials=True, load_data=True):
        """
        This dataset is small enough to be stored in-memory, so load all point clouds here
        """
        PointCloudDataset.__init__(self, 'SsePCD')
        ...
        self.pcd_path = join(self.path, 'original_pcd')  # ../../Data/SsePCD/original_pcd
        self.ply_path = join(self.path, self.train_path)  # ../../Data/SsePCD/original_ply
        ...
        self.prepare_SsePCD_ply()  # ../../Data/SsePCD/original_ply/***.ply
        print('self.cloud_names =', self.cloud_names)
        print('self.all_splits =', self.all_splits)
        ...
        #     pass
        # else:
        #     raise ValueError('subsampling_parameter too large (should be under 1 mm')
    ...
    def prepare_SsePCD_ply(self):

        print('\nPreparing ply files')
        t0 = time.time()

        # Folder for the ply files
        if not exists(self.ply_path):
            makedirs(self.ply_path)

        for file in listdir(self.pcd_path):  # xxx.pcd
            if file[-4:] == '.pcd':
                self.cloud_names.append(file[0:-4])
                ply_cloud_file = join(self.ply_path, file[0:-4] + '.ply')
                if exists(ply_cloud_file):
                    continue
    ...
    def pcd_file_parse(self, pcd_file_path: str):
        pcd_header = {}
            ...
            while True:
                ...
                if not data_flag:
                    if ln.startswith('VERSION'):
                        pcd_header['VERSION'] = ln.split(' ')[1:]
                    elif ln.startswith('FIELDS'):  # 每一个维度字段名
                        pcd_header['FIELDS'] = ln.split(' ')[1:]
                    elif ln.startswith('SIZE'):  # 每一个维度的大小
                        pcd_header['SIZE'] = [int(sz) for sz in ln.split(' ')[1:]]
                    elif ln.startswith('TYPE'):  # 每一个维度的类型 (I, U, F)
                        pcd_header['TYPE'] = ln.split(' ')[1:]
                    elif ln.startswith('COUNT'):  # 每一个维度包含的元素数目
                        pcd_header['COUNT'] = [int(cnt) for cnt in ln.split(' ')[1:]]
                    elif ln.startswith('WIDTH'):  # 点云数据集的宽度
                        pcd_header['WIDTH'] = int(ln.split(' ')[1])
                    elif ln.startswith('HEIGHT'):  # 点云数据集的高度
                        pcd_header['HEIGHT'] = int(ln.split(' ')[1])
                    elif ln.startswith('POINTS'):  # 点云中点的总数
                        pcd_header['POINTS'] = int(ln.split(' ')[1])
                    elif ln.startswith('VIEWPOINT'):  # 点云的获取视点
                        pcd_header['VIEWPOINT'] = [float(vp) for vp in ln.split(' ')[1:]]
                    elif ln.startswith('DATA'):
                        pcd_header['DATA'] = ln.split(' ')[1]
                        if pcd_header['DATA'] != 'ascii':
                            raise ValueError('pcd DATA is not ascii')
                        data_flag = True
                else:
                    ...
                    cloud_points = np.vstack((cloud_points, np.array(point).astype(np.float32)))
                    cloud_colors = np.vstack((cloud_colors, np.array(color).astype(np.uint8)))
                    if label is not None:
                        cloud_classes = np.vstack((cloud_classes, np.array([label]).astype(np.int32)))
                    else:
                        raise ValueError('pcd DATA does not have label field')
    ...
    def load_subsampled_clouds(self):
        ...
        tree_path = join(self.path, 'input_{:.4f}'.format(dl))  # ../../Data/SsePCD/input_0.001
        ...
        if self.set in ['validation', 'test']:
            ...
            for i, file_path in enumerate(self.files):
                ...
                proj_file = join(tree_path, '{:s}_proj.pkl'.format(cloud_name))  # Area_5_proj.pkl
...
class SsePCDSampler(Sampler):
    """Sampler for SsePCD"""

    def __init__(self, dataset: SsePCDDataset):
...
class SsePCDCustomBatch:
    """Custom batch definition with memory pinning for SsePCD"""
...
def SsePCDCollate(batch_data):
    return SsePCDCustomBatch(batch_data)
